...
function init(  ) {
	...
	// Cubes
	var size_x = Math.floor(Math.random() * 800) + 150;
	var size_y = Math.floor(Math.random() * 250) + 150;
	var size_z = Math.floor(Math.random() * 350) + 150;
	var geometry = new THREE.BoxGeometry(size_x,size_y,size_z);
	
	
	var material = new THREE.MeshLambertMaterial( { color: "white", overdraw: 0 } );
	var count = Math.floor(Math.random() * 70) + 35;
